...
def check_checkmate(board):
    ...
    for i in range(0,len(black_pieces)):
        ...
        if not type(valid_moves(piece_to_move[0],piece_to_move[1],board)) == int:
            for i in valid_moves(piece_to_move[0],piece_to_move[1],board):
...
def check_for_check(board,moust):
    ...
    for piece in pieces:
        x1,y1 = piece
        if board.game[x1][y1][1] == p.black and white in valid_moves(x1,y1,board):
            x , y = white
            if path_is_clear(x1,y1,x,y,board):
                return ["yes" , black , white]
        if board.game[x1][y1][1] == p.white and black in valid_moves(x1,y1,board):
            x , y = black
            if path_is_clear(x1,y1,x,y,board):
...
def possibleblack(boaard, check):
    ...
    all_possible_moves = [[k,m] for k in black_pieces for m in valid_moves(k[0],k[1],boaard)]
    ...
    for move in all_possible_moves:
        ...
        i = move[1]
        if not type(valid_moves(piece_to_move[0],piece_to_move[1],boaard)) == int:      
            if piece_to_move[0] > -1 and piece_to_move[1] > -1 and  i[0] > -1 and i[1] > -1 and i[0] < 8 and i[1] < 8:
                            ...
                            if boaard.game[piece_to_move[0]][piece_to_move[1]][1] == p.white:
                                ...
                                if [i[0],i[1]] == [7,6]:
                                    ...
                                    for piece in pieces_black:
                                        x1,y1 = piece
                                        if [7,5] in valid_moves(x1,y1,boaard):
                                            x , y = [7,5]
                                    ...
                                    for piece in pieces_black:
                                        x1,y1 = piece
                                        if boaard.game[x1][y1][1] == p.black  and ([7,1] in valid_moves(x1,y1,boaard) or [7,3] in valid_moves(x1,y1,boaard)):
                                            x , y = [7,1]
                                            if path_is_clear(x1,y1,x,y,boaard):
                                                cool = False
                                            x,y = [7,3]
                            ...
                            if boaard.game[piece_to_move[0]][piece_to_move[1]][1] == p.black:
                                if [i[0],i[1]] == [0,6]:
                                    ...
                                    for piece in pieces_white:
                                        x1,y1 = piece
                                        if [0,5] in valid_moves(x1,y1,boaard):
                                            x , y = [0,5]
                                            if path_is_clear(x1,y1,x,y,boaard):
                                                cool = False
                                    if cool == False:
                                    ...
                                    for piece in pieces_white:
                                        x1,y1 = piece
                                        if boaard.game[x1][y1][1] == p.white  and ([0,1] in valid_moves(x1,y1,boaard) or [0,3] in valid_moves(x1,y1,boaard)):
                                            x , y = [0,1]
                                            if path_is_clear(x1,y1,x,y,boaard):
                                                cool = False
                                            x,y = [0,3]
    ...
    while True:
        ...
        if check_checkmate(boaard):
            if check_for_check(boaard,-1)[0]:
                ...
                sys.exit(0)
        e = 0
        move = best_move_generator(boaard,possible,3)
        xl = move[0][0]
        yl = move[0][1]
        xl2 = move[1][0]
        yl2 = move[1][1]
...
def best_move_generator(board,legal_moves,depth = 3):
    ...
    for move in legal_moves:
        ...
        if move_count < 100 and best_eval >-1:
            print(True)
            break
        if cnt == 2 and best_eval > 0:
            break
        if cnt == 3 and best_eval > -1:
            break
...
def min_max(board, depth, alpha, beta, maximizing_player):
    ...
    all_possible_moves = dict()
    all_possible_moves = [[k,m] for k in black_pieces for m in valid_moves(k[0],k[1],board)]
    ...
    for move in all_possible_moves:
        ...
        if not type(valid_moves(piece_to_move[0],piece_to_move[1],board)) == int:      
            if piece_to_move[0] > -1 and piece_to_move[1] > -1 and  i[0] > -1 and i[1] > -1 and i[0] < 8 and i[1] < 8:
                if path_is_clear(piece_to_move[0] , piece_to_move[1] , i[0] ,i[1] ,board):
                        if board.game[piece_to_move[0]][piece_to_move[1]] != [0,0]: 
                        # move implementation
                            if board.game[piece_to_move[0]][piece_to_move[1]][0] == p.king:
                                    ...
                                    if board.game[piece_to_move[0]][piece_to_move[1]][1] == p.white:
                                        ...
                                        if [i[0],i[1]] == [7,6]:
                                            ...
                                            for piece in pieces_black:
                                                x1,y1 = piece
                                                if [7,5] in valid_moves(x1,y1,board):
                                                    x , y = [7,5]
                                                    if path_is_clear(x1,y1,x,y,board):
                                                        cool = False
                                            if cool == False:
                                            ...
                                            for piece in pieces_black:
                                                x1,y1 = piece
                                                if board.game[x1][y1][1] == p.black  and ([7,1] in valid_moves(x1,y1,board) or [7,3] in valid_moves(x1,y1,board)):
                                                    x , y = [7,1]
                                                    if path_is_clear(x1,y1,x,y,board):
                                                        cool = False
                                                    x,y = [7,3]
                                    ...
                                    if board.game[piece_to_move[0]][piece_to_move[1]][1] == p.black:
                                        if [i[0],i[1]] == [0,6]:
                                            ...
                                            for piece in pieces_white:
                                                x1,y1 = piece
                                                if [0,5] in valid_moves(x1,y1,board):
                                                    x , y = [0,5]
                                                    if path_is_clear(x1,y1,x,y,board):
                                                        cool = False
                                            if cool == False:
                                            ...
                                            for piece in pieces_white:
                                                x1,y1 = piece
                                                if board.game[x1][y1][1] == p.white  and ([0,1] in valid_moves(x1,y1,board) or [0,3] in valid_moves(x1,y1,board)):
                                                    x , y = [0,1]
                                                    if path_is_clear(x1,y1,x,y,board):
                                                        cool = False
                                                    x,y = [0,3]
    ...
    black_legal_moves = [i for i in legal_moves if board.game[i[0][0]][i[0][1]][1] == p.black]
    white_legal_moves = [i for i in legal_moves if board.game[i[0][0]][i[0][1]][1] == p.white]
...
def score_move(move, board):
    ...
    if board.game[x][y] != [0, 0]:
        score = 6  # High score for capturing moves
    if board.game[i][j][0] == p.queen:
        score = -1
    if board.game[i][j][0] == p.rook:
        score = -1
    ...
    if board.game[i][j][0] == p.pawn and x in [0, 7]:
        score = 4  # Highest score for pawn promotion
    if move == [[0,4],[0,6]] or move == [[0,4],[0,2]]:
        score = 5
    lelo = board.copy_board()
    lelo.move(i,j,x,y,lelo,1,False)
    if check_for_check(lelo,-1)[0] == "yes":
        score = 5
    if move == [[1,4],[3,4]]:
        score = 10
    try:
        if board.game[x+1][y+1][0] == p.pawn:
            score = -1
    except IndexError:
        pass
    try:
